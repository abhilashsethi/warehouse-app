...
import AddNurseSchemas from "schemas/AddNurseSchemas";
...
const AddNurseDrawer = ({ open, onClose, mutate }: Props) => {
	const [countryDetails, setCountryDetails] = useState({
		code: "IN",
		label: "India",
		phone: "91",
	});
	const initialValues = AddNurseSchemas.reduce((accumulator, currentValue) => {
		accumulator[currentValue.name] = currentValue.initialValue;
		return accumulator;
	}, {} as any);
	const validationSchema = AddNurseSchemas.reduce(
			...
			<Drawer anchor="right" open={open} onClose={() => onClose && onClose()}>
				...
				>
					...
					>
						{(formik) => (
							<Form>
								{AddNurseSchemas?.map((inputItem, index) => (
									<div key={index}>
