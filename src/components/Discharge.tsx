import DischargeSchemas from "schemas/DischargeSchemas";
...
const Discharge = () => {
	...
	const validationSchema = DischargeSchemas?.reduce(
				...
				{(formik) => (
					<Form>
						<div className="grid grid-cols-2 gap-4 w-full">
							{DischargeSchemas?.map((inputItem, index) => (
								<div key={index}>
									{inputItem?.name === "doctor" ? (
										<div className=" w-full">
											<DoctorSelecter />
										</div>
									) : inputItem?.name === "bloodGroup" ? (
										<div className=" w-full">
											<BloodGroupSelector icon={<Bloodtype />} />
										</div>
									) : (
										<div
											className={
