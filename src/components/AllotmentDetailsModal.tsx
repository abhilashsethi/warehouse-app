...
import DoctorViewDetails from "./DoctorViewDetails";
type props = {
	title: string;
...
export default ({ title, isInternship = false }: props) => {
	...
	return (
		<>
			...
			>
				<Tooltip title="Allotment Details">
					<Avatar
						variant="rounded"
			...
			<section
				className={`fixed ml-10 inset-0 z-50 grid place-content-center p-2 ${
					showRequestForm ? "opacity-100" : "pointer-events-none opacity-0"
				}`}
			>
				<div
					className={`z-50 mx-auto transition duration-300 ${
						showRequestForm ? "scale-100" : "scale-0"
					}`}
				>
					<div className="flex flex-wrap gap-2 overflow-y-scroll rounded-xl bg-white shadow-lg shadow-purple-50/20 lg:flex-nowrap">
						<div className="flex flex-col gap-4 rounded-xl p-3 lg:rounded-r-xl">
							<div className="py-3 px-2">
								...
								<AllotmentDetails
								// close={() => setShowRequestForm(false)}
								// onClose={() => setShowRequestForm((prev) => !prev)}
								// title={title}
								// refresh={showRequestForm}
